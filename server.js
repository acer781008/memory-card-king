...
io.on('connection',s=>{
 s.on('createRoom',(settings,cb)=>{let c;do c=code();while(rooms.has(c));let r={code:c,settings,state:'waiting',players:new Map(),timer:null,countdownTimer:null,gameTicker:null};rooms.set(c,r);s.join(c);s.data.admin=c;emit(r);cb?.({ok:true,code:c});});
