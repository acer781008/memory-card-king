...
s.on('room',r=>{if(room&&r.code!==room)return;$('count').textContent=r.online??r.players.length;$('state').textContent={waiting:'等待玩家加入',preparing:`等待素材載入 ${r.ready||0}/${r.readyTotal||r.online||0}`,countdown:'準備開始・倒數中',playing:'遊戲進行中',finished:'本局已結束'}[r.state]||r.state;if(r.state==='waiting'){$('startTimer').textContent='--';$('gameTimer').textContent='--:--'}else if(r.state==='preparing'){$('startTimer').textContent='等待素材';$('gameTimer').textContent='--:--'}});
s.on('prepareStatus',x=>{if(!x)return;$('state').textContent=`等待素材載入 ${x.ready}/${x.total}`;$('startTimer').textContent='等待素材'});
