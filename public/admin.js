...
$('copyShare').onclick=()=>{if(!room)return notice('copyMsg','請先產生房號');let a=settings(),t={farm:'種菜素材',idiom:'成語',poem:'唐詩'}[a.type];copyText(`🃏 翻牌記憶王\n房號：${room}\n玩法：${t}｜${a.difficulty}｜${a.cards}張\n開始倒數：${a.startCountdown}秒\n遊戲時間：${a.gameMinutes}分鐘\n完成條件：${a.finishGoal==='all'?'全部玩家完成':`前${a.finishGoal}位完成`}\n備註：${a.note||'無'}\n玩家連結：${url()}`)};
...
s.on('gameTime',ms=>{$('state').textContent='遊戲進行中';$('startTimer').textContent='已開始';$('gameTimer').textContent=fmt(ms)});
...
s.on('finished',()=>{$('state').textContent='本局已結束';$('startTimer').textContent='—';$('gameTimer').textContent='00:00'});
